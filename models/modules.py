...
class MixtureExperts(nn.Module):
    def __init__(self, number_of_experts: int, top_k: int, input_dim, output_dim):
        super(MixtureExperts, self).__init__()
        self.number_of_experts = number_of_experts
        self.experts = nn.ModuleList([nn.Linear(input_dim, output_dim) for i in range(number_of_experts)])
        self.topk = top_k
        self.gating = nn.Linear(input_dim, number_of_experts, bias=False)
        self.noise = nn.Linear(input_dim, number_of_experts, bias=False)

    def topK(self, input: torch.Tensor, topk: int):

        topk_values, topk_indices = torch.topk(input, topk)
        mask = torch.full_like(input, float('-inf'))
        result = torch.where(input >= topk_values[..., -1:], input, mask)
        return result

    def add_noise(self, input):
        noise_input = torch.nn.functional.softplus(self.noise(input))   #shape: B*K
        random_tensor = torch.randn(noise_input.shape).to(noise_input.device)  #shape: B*K
        return random_tensor * noise_input
    ...
    def forward(self, input1: torch.Tensor, input2: torch.Tensor):
        ...
        attn_1 = self.gating(input1) + self.add_noise(input1)    # shape: B*K
        # attn_1 = self.gating(input1) # shape: B*K
        attn_2 = self.gating(input2) + self.add_noise(input2)    # shape: B*K
        # attn_2 = self.gating(input2) # shape: B*K

        mask_attn1 = self.topK(attn_1, self.topk)
        mask_attn2 = self.topK(attn_2, self.topk)

        softmax_attn1 = torch.nn.functional.softmax(mask_attn1, dim=1)
        softmax_attn2 = torch.nn.functional.softmax(mask_attn2, dim=1)


        output_1, output_2 = [],[]
        for layer_i, module_i in enumerate(self.experts):
            output_1.append(module_i(input1))
            output_2.append(module_i(input2))

        output_1 = torch.stack(output_1, dim=2)
        output_2 = torch.stack(output_2, dim=2)

        out_1 = torch.matmul(output_1, softmax_attn1.unsqueeze(dim=2)).squeeze(dim=2) + input1
        out_2 = torch.matmul(output_2, softmax_attn2.unsqueeze(dim=2)).squeeze(dim=2) + input2

        assert out_1.shape == input1.shape
        assert out_2.shape == input2.shape
...
class MLPEdgeClassifier(nn.Module):
    def __init__(self, input_dim: int, dropout: float = 0.1):
        ...
        """
        super().__init__()
        self.fc1 = nn.Linear(input_dim*2, 80)
        self.fc2 = nn.Linear(80, 10)
        self.fc3 = nn.Linear(10, 1)
        self.act = nn.ReLU()
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor, input_2: torch.Tensor):
        """
        ...
        """
        # Tensor, shape (*, 80)
        x = self.dropout(self.act(self.fc1(torch.cat([x, input_2], dim=1))))
        # Tensor, shape (*, 10)
