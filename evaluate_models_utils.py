...
from utils.utils import set_random_seed, HistoricalNeighborSampler
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler,
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'RepeatMixer', 'DyGFormer', 'FreeDyG']:
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if model_name in ['RepeatMixer']:
                positive_probabilities = model[1](input_1=batch_edge_embeddings,
                                                  input_2=None, input_3=None).squeeze(dim=-1).sigmoid()
                negative_probabilities = model[1](batch_neg_edge_embeddings,
                                                  input_2=None, input_3=None).squeeze(dim=-1).sigmoid()
            else:
                positive_probabilities = model[1](input_1=batch_src_node_embeddings,
                                                  ...
                                                  input_3=None if model_name not in [
                                                      'RepeatMixer'] else batch_edge_embeddings).squeeze(
                    dim=-1).sigmoid()
                negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings,
                                                  input_2=batch_neg_dst_node_embeddings,
                                                  # input_3=None).squeeze(dim=-1).sigmoid()
                                                  input_3=None if model_name not in [
                                                      'RepeatMixer'] else batch_neg_edge_embeddings).squeeze(
                    dim=-1).sigmoid()
