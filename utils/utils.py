...
class NeighborSampler:
    ...
    def get_single_sequence(self, node_ids: np.ndarray, node_interact_times: np.ndarray, dst_ids: np.ndarray):
        """
        get historical neighbors of nodes in node_ids at the first hop with max_num_neighbors as the maximal number of neighbors (make the computation feasible)
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        ...
        nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list = [], [], []
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time, dst_id) in enumerate(zip(node_ids, node_interact_times, dst_ids)):
            # find neighbors that interacted with node_id before time node_interact_time
            # print(node_id, node_interact_time, dst_id)
            node_neighbor_ids, node_edge_ids, node_neighbor_times, _ = self.find_neighbors_before(node_id=node_id,
                                                                                                  interact_time=node_interact_time,
                                                                                                  return_sampled_probabilities=False,
                                                                                                  )
            if len(node_edge_ids) > 0:
                assert len(node_neighbor_times) > 0

                id_sequence = np.concatenate((np.array([node_id]), node_neighbor_ids))
                history_edge_ids = np.concatenate((np.array([node_edge_ids[0]]), node_edge_ids))
                history_times = np.concatenate((np.array([node_neighbor_times[0]]), node_neighbor_times))
            else:
                id_sequence = np.array([node_id])
                history_edge_ids = np.array([-1])
                history_times = np.array([node_interact_time])
            id_sequence = np.concatenate((id_sequence, np.array([dst_id])))
            history_edge_ids = np.concatenate((history_edge_ids, np.array([-1])))
            history_times = np.concatenate((history_times, np.array([node_interact_time])))

            assert len(id_sequence) >= 2
            assert len(history_edge_ids) == len(id_sequence) == len(history_times)
            assert len(history_times) >= 2
            nodes_neighbor_ids_list.append(id_sequence)
            nodes_edge_ids_list.append(history_edge_ids)
            nodes_neighbor_times_list.append(history_times)
...
def get_historical_neighbor_sampler(data: pd.DataFrame, sample_neighbor_strategy: str = 'uniform',
...
def get_neighbor_sampler(data: pd.DataFrame, sample_neighbor_strategy: str = 'uniform',
                         time_scaling_factor: float = 0.0, seed: int = None):
    """
    get neighbor sampler
    :param data: DataFrame
...
class NegativeEdgeSampler(object):
    ...
    def historical_sample(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray,
        ...
        # Hence, convert the type to int64 to guarantee valid index
