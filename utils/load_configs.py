...
def get_link_prediction_args(is_evaluation: bool = False):
    ...
    parser.add_argument('--dataset_name', type=str, help='dataset to be used', default='USLegis',
                        choices=['wikipedia', 'reddit', 'mooc', 'lastfm', 'enron', 'SocialEvo', 'uci', 'new_Flights', 'CanParl', 'USLegis', 'UNtrade', 'UNvote', 'Contacts'])
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    parser.add_argument('--normalized', type=str, default='z_score', help='type of normalized')
    ...
    parser.add_argument('--num_layers', type=int, default=0, help='number of model layers')
    ...
    parser.add_argument('--number_of_exp', type=int, default=8, help='number of experts')
    parser.add_argument('--topk', type=int, default=2, help='number of activating experts')
...
def load_link_prediction_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
    ...
    elif args.model_name in ['JODIE', 'DyRep', 'edge_TGN', 'TGN']:
        ...
        if args.model_name == 'JODIE':
            ...
            assert args.model_name in['TGN', 'edge_TGN']
            if args.dataset_name in ['mooc', 'UNtrade']:
                ...
                args.dropout = 0.1
        if args.model_name in ['TGN', 'DyRep', 'edge_TGN']:
            if args.dataset_name in ['CanParl'] or (args.model_name in ['TGN', 'edge_TGN'] and args.dataset_name == 'UNvote'):
                args.sample_neighbor_strategy = 'uniform'
            else:
                args.sample_neighbor_strategy = 'recent'
        ...
        if args.dataset_name in ['reddit', 'CanParl', 'USLegis', 'UNtrade', 'UNvote']:
            ...
            args.sample_neighbor_strategy = 'recent'
    elif args.model_name in ['GraphMixer', 'RepeatMixer']:
        args.num_layers = 3
        if args.dataset_name in ['wikipedia']:
            args.num_neighbors = 30
        elif args.dataset_name in ['reddit', 'lastfm', 'enron', 'SocialEvo']:
            args.num_neighbors = 32
        ...
        if args.dataset_name in ['reddit']:
            args.max_input_sequence_length = 64
            args.patch_size = 1
        elif args.dataset_name in ['enron', 'wikipedia']:
...
def get_node_classification_args():
    ...
    parser.add_argument('--batch_size', type=int, default=200, help='batch size')
    parser.add_argument('--model_name', type=str, default='DynTrans', help='name of the model',
                        choices=['JODIE', 'DyRep', 'TGAT', 'edge_TGN', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'RepeatMixer','DyGFormer', 'DynTrans'])
...
def load_node_classification_best_configs(args: argparse.Namespace):
    ...
    if args.model_name == 'TGAT':
        ...
        if args.dataset_name in ['reddit']:
            ...
            args.sample_neighbor_strategy = 'recent'
    elif args.model_name in ['JODIE', 'DyRep', 'TGN', 'edge_TGN']:
        args.num_neighbors = 10
        args.num_layers = 1
        args.dropout = 0.1
        args.sample_neighbor_strategy = 'recent'
